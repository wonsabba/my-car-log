...
export default function Home() {
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen bg-white max-w-6xl mx-auto overflow-hidden border-x border-slate-200 font-sans">
      ...
      {toast && (
        <div className="fixed bottom-20 left-1/2 -translate-x-1/2 z-50 px-6 py-2 rounded-full shadow-xl bg-slate-800 text-white font-bold text-xs">{toast.msg}</div>
      )}
          ...
          <section className={`p-4 rounded-2xl border ${editingId ? 'bg-orange-50 border-orange-200' : 'bg-slate-50 border-slate-100'}`}>
            <form onSubmit={handleSave} className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[10px] font-bold text-slate-600 ml-1 mb-1 block uppercase">Date</label>
                ...
                <div>
                  <label className="text-[10px] font-bold text-slate-600 ml-1 mb-1 block uppercase">Brand</label>
              ...
              <div className="grid grid-cols-3 gap-2">
                <div>
                  <label className="text-[10px] font-bold text-slate-600 ml-1 mb-1 block uppercase">Price</label>
                ...
                <div>
                  <label className="text-[10px] font-bold text-slate-600 ml-1 mb-1 block uppercase">Liter</label>
                ...
                <div>
                  <label className="text-[10px] font-black text-blue-600 ml-1 mb-1 block uppercase">Amount</label>
              ...
              <div className="grid grid-cols-2 gap-3 items-end">
                <div>
                  <label className="text-[10px] font-bold text-slate-600 ml-1 mb-1 block uppercase">Distance</label>
                ...
                <button type="submit" className={`py-2 rounded-xl font-black text-white shadow-md transition-all ${editingId ? 'bg-orange-600' : 'bg-slate-900'}`}>{editingId ? "수정" : "저장"}</button>
              </div>
            </form>
          </section>
        </div>
      ...
      <main className="flex-1 flex flex-col min-h-0 bg-white relative">
        <div className="sticky top-0 bg-white z-10 border-b border-slate-100 shrink-0">
          <div className="px-5 py-4 flex justify-between items-center bg-white">
            <div className="flex items-center gap-2">
              ...
              {/* [NEW] 타이틀 바로 옆 심플 엑셀 아이콘 */}
              <button 
                onClick={downloadExcel}
                className="p-1 hover:bg-slate-100 rounded-md transition-colors group relative"
                title="엑셀 다운로드"
              >
        ...
        <div className="sticky bottom-0 bg-slate-900 text-white px-4 py-3 flex items-center shadow-[0_-4px_15px_rgba(0,0,0,0.15)] z-10 shrink-0 font-bold">
          <div className="flex-1 text-[10px] font-black text-slate-400 text-center uppercase tracking-widest leading-none tracking-tighter">Summary</div>
