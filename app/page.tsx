...
export default function Home() {
  ...
  const brandConfig: { [key: string]: { name: string; color: string } } = {
    "1": { name: "SK", color: "text-red-600" },
    "2": { name: "GS", color: "text-emerald-600" },
    "3": { name: "알뜰", color: "text-orange-500" }
  };
  ...
  const downloadExcel = () => {
    ...
    const headers = ["주유일자", "주유회사", "단가(원)", "주유량(L)", "주유액(원)", "주행거리(Km)"];
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    const payload = { ...formData, unit_price_krw: Number(formData.unit_price_krw), fuel_volume_l: Number(formData.fuel_volume_l), amount_krw: Number(formData.amount_krw), distance_km: Number(formData.distance_km) };
    if (editingId) {
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen bg-white max-w-6xl mx-auto overflow-hidden border-x border-slate-200 font-sans">
          ...
          <section className={`p-4 rounded-2xl border ${editingId ? 'bg-orange-50 border-orange-200' : 'bg-slate-50 border-slate-100'}`}>
            <form onSubmit={handleSave} className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[10px] font-bold text-slate-600 ml-1 mb-1 block uppercase tracking-tighter">Date</label>
                ...
                <div>
                  <label className="text-[10px] font-bold text-slate-600 ml-1 mb-1 block uppercase tracking-tighter">Brand</label>
              ...
              <div className="grid grid-cols-3 gap-2">
                <div>
                  <label className="text-[10px] font-bold text-slate-600 ml-1 mb-1 block uppercase tracking-tighter">Price</label>
                ...
                <div>
                  <label className="text-[10px] font-bold text-slate-600 ml-1 mb-1 block uppercase tracking-tighter">Liter</label>
                ...
                <div>
                  <label className="text-[10px] font-black text-blue-600 ml-1 mb-1 block uppercase tracking-tighter">Amount</label>
              ...
              <div className="grid grid-cols-2 gap-3 items-end">
                <div>
                  <label className="text-[10px] font-bold text-slate-600 ml-1 mb-1 block uppercase tracking-tighter">Distance</label>
      ...
      {/* --- [우측/하단 목록: 모바일 대응 최적화] --- */}
      <main className="flex-1 flex flex-col min-h-0 bg-white relative overflow-x-auto">
        <div className="min-w-[360px] flex flex-col h-full shrink-0"> {/* min-w 부여로 레이아웃 붕괴 방지 */}
          <div className="sticky top-0 bg-white z-10 border-b border-slate-100 shrink-0">
            <div className="px-5 py-4 flex justify-between items-center bg-white">
              <div className="flex items-center gap-2">
                <h2 className="text-lg font-black text-slate-800 tracking-tight leading-none">주유 내역 목록</h2>
                <button onClick={downloadExcel} className="p-1 hover:bg-slate-100 rounded-md transition-colors group" title="엑셀 다운로드">
            ...
            <div className="bg-slate-50 px-3 py-2 flex items-center border-t border-slate-200 text-[10px] font-black text-slate-500 uppercase tracking-tight whitespace-nowrap">
              <div className="flex-1 text-center pr-2">주유일자</div>
              <div className="w-[45px] text-center shrink-0">회사</div>
              <div className="w-[55px] text-right pr-2 shrink-0">단가</div>
              <div className="w-[55px] text-right pr-2 shrink-0 border-r border-slate-300">주유량</div>
              <div className="w-[75px] text-right pr-2 shrink-0">주유액</div>
              <div className="w-[70px] text-right pr-3 shrink-0">거리</div>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto divide-y divide-slate-100 custom-scrollbar">
            {logs.map((log) => (
              <div key={log.id} onDoubleClick={() => startEdit(log)} className={`flex items-center px-3 py-4 hover:bg-slate-50 cursor-pointer transition-colors whitespace-nowrap ${editingId === log.id ? 'bg-orange-50 ring-1 ring-inset ring-orange-200' : ''}`}>
                ...
                <div className={`w-[45px] shrink-0 text-sm font-black text-center tracking-tighter ${brandConfig[log.brand]?.color || "text-slate-950"}`}>{brandConfig[log.brand]?.name || "-"}</div>
                ...
                <div className="w-[70px] shrink-0 text-sm font-black text-blue-700 text-right pr-3 tracking-tighter">{log.distance_km.toLocaleString()}</div>
              </div>
            ))}
          </div>
          ...
          <div className="sticky bottom-0 bg-slate-900 text-white px-3 py-3 flex items-center shadow-[0_-4px_15px_rgba(0,0,0,0.15)] z-10 shrink-0 font-bold whitespace-nowrap">
            <div className="flex-1 text-[9px] font-black text-slate-400 text-center uppercase tracking-tighter pr-2 leading-none">Summary</div>
            ...
            <div className="w-[70px] shrink-0 text-right pr-3 text-slate-500 text-[9px] self-center uppercase tracking-tighter leading-none">Total Results</div>
          </div>
        </div>
      </main>
