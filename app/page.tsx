...
export default function Home() {
  ...
  // [기존 함수들: downloadExcel, fetchLogs, handleSave, resetForm, handleDelete, startEdit 등 동일]
    ...
    return (
      <div className="flex items-center justify-center h-screen bg-slate-100 font-sans p-6">
        <div className="w-full max-w-sm bg-white p-8 rounded-3xl shadow-2xl">
          <h1 className="text-2xl font-black text-slate-800 mb-6 text-center">Brandon</h1>
          <form onSubmit={handleLogin} className="space-y-4">
            <input type="email" placeholder="이메일" className="w-full p-4 border border-slate-200 rounded-2xl outline-none focus:border-blue-500 font-bold"
              value={email} onChange={(e) => setEmail(e.target.value)} required />
            <input type="password" placeholder="비밀 password" className="w-full p-4 border border-slate-200 rounded-2xl outline-none focus:border-blue-500 font-bold"
              value={password} onChange={(e) => setPassword(e.target.value)} required />
            <button type="submit" className="w-full py-4 bg-slate-900 text-white rounded-2xl font-black text-lg shadow-lg active:scale-95 transition-all">로그인</button>
          </form>
      ...
      <main className="flex-1 flex flex-col min-h-0 bg-white relative overflow-x-auto font-sans">
        <div className="min-w-[360px] flex flex-col h-full shrink-0">
          ...
          <div className="sticky bottom-0 bg-slate-900 text-white px-3 py-3 flex items-center shadow-[0_-4px_15px_rgba(0,0,0,0.15)] z-10 shrink-0 font-bold whitespace-nowrap">
            ...
            <div className="w-[70px] shrink-0 pr-3 text-slate-500 text-[9px] self-center tracking-tighter leading-none"></div>
          </div>
        </div>
      </main>
